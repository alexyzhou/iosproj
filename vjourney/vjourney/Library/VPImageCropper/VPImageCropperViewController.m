...
- (id)initWithImage:(UIImage *)originalImage fullFrame:(CGRect)fullFrame cropFrame:(CGRect)cropFrame limitScaleRatio:(NSInteger)limitRatio orientation:(UIImageOrientation)orientation {
    self = [super init];
    if (self) {
...
- (void)initView {
    self.showImgView = [[UIImageView alloc] initWithFrame:_fullFrame];
...
- (void)confirm:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(VPImageCropperDelegate)]) {
        [self.delegate imageCropper:self didFinished:[self getSubImage2]];
    }
}
...
-(UIImage*) rotateImage:(UIImage*) src WithImageOrientation:(UIImageOrientation)orientation
{
...
-(UIImage *)getSubImage1{
    ...
    return [self rotateImage:smallImage WithImageOrientation:_orientation];
}
...
-(UIImage *)getSubImage2{
    ...
    rect1.origin.x += 1.0f;
    rect1.origin.y += 1.0f;
    rect1.size.width -= 2.0f;
    rect1.size.height -= 2.0f;
