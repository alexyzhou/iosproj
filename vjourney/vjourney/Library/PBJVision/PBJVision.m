...
- (void)_setupSession
{
    ...
    if (newCaptureOutput == _captureOutputVideo) {
        ...
        //[_currentDevice setActiveVideoMinFrameDuration:frameDuration];
        //[_currentDevice setActiveVideoMaxFrameDuration:frameDuration];
...
- (void)startVideoCapture
{
    ...
    [self _enqueueBlockInCaptureVideoQueue:^{
        ...
        video_count++;
        NSString *outputPath = [NSString stringWithFormat:@"%@%@", [VJNYUtilities videoCaptureTmpFolderPath], [NSString stringWithFormat:@"%d.mp4",video_count]];
...
-(void)switchFlashLight {
    if (_currentDevice == _captureDeviceBack) {
        
        [_currentDevice lockForConfiguration:nil];
        
        if (_currentDevice.torchMode == AVCaptureTorchModeOn) {
